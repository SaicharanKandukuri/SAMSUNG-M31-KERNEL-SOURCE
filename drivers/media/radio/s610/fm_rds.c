					   ...
					   fm_host_rds_errors_enum errors)
{
	...
	buf_ptr[HOST_RDS_BLOCK_FMT_STATUS] =
		(blk_type << HOST_RDS_DATA_BLKTYPE_POSI) | (errors << HOST_RDS_DATA_ERRORS_POSI) | HOST_RDS_DATA_AVAIL_MASK;
					   ...
					   fm_rds_block_type_enum blk_type, fm_host_rds_errors_enum errors)
{
	...
	if (radio->low->rds_buffer->outdex > radio->low->rds_buffer->index)
		usage = radio->low->rds_buffer->size - radio->low->rds_buffer->outdex + radio->low->rds_buffer->index;
	else
		usage = radio->low->rds_buffer->index - radio->low->rds_buffer->outdex;

	if ((capa - usage) >= (HOST_RDS_BLOCK_SIZE * 4))
	{
		buf_ptr = radio->low->rds_buffer->base + radio->low->rds_buffer->index;

		buf_ptr[HOST_RDS_BLOCK_FMT_LSB] = (u8)(rds_data & 0xff);
		buf_ptr[HOST_RDS_BLOCK_FMT_MSB] = (u8)(rds_data >> 8);
		buf_ptr[HOST_RDS_BLOCK_FMT_STATUS] = (blk_type
											  << HOST_RDS_DATA_BLKTYPE_POSI) |
											 (errors << HOST_RDS_DATA_ERRORS_POSI) | HOST_RDS_DATA_AVAIL_MASK;

		/* Advances the buffer's index */
		radio->low->rds_buffer->index += HOST_RDS_BLOCK_SIZE;
...
u16 fm_rds_get_avail_bytes(struct s610_radio *radio)
{
	...
	if (radio->low->rds_buffer->outdex >
		radio->low->rds_buffer->index)
		avail_bytes = (radio->low->rds_buffer->size - radio->low->rds_buffer->outdex + radio->low->rds_buffer->index);
	else
		avail_bytes = (radio->low->rds_buffer->index - radio->low->rds_buffer->outdex);
					 ...
					 u16 *blocks)
{
	...
	buf_ptr = radio->low->rds_buffer->base + radio->low->rds_buffer->outdex;
	...
	{
		buffer[(avail_blocks - 1) * HOST_RDS_BLOCK_SIZE + HOST_RDS_BLOCK_FMT_STATUS] &=
	...
	/* Update RDS flags */
	if ((avail_bytes / HOST_RDS_BLOCK_SIZE) < radio->low->fm_state.rds_mem_thresh)
		fm_clear_flag_bits(radio, FLAG_BUF_FUL);
						 ...
						 fm_rds_state_enum new_state)
{
	...
	if ((old_state == RDS_STATE_FULL_SYNC) && (new_state == RDS_STATE_HAVE_DATA))
	{
		fm_update_rds_sync_status(radio, FALSE); /* unsynced */
	}
	else if ((old_state != RDS_STATE_FULL_SYNC) && (new_state == RDS_STATE_FULL_SYNC))
	{
...
void fm_rds_update_error_status(struct s610_radio *radio, u16 errors)
{
	...
	if (radio->low->fm_rds_state.error_blks >= radio->low->fm_state.rds_unsync_blk_cnt)
	{
		if (radio->low->fm_rds_state.error_bits >= radio->low->fm_state.rds_unsync_bit_cnt)
		{
			/* sync-loss */
	...
	u16 err_count)
{
	...
	else if ((err_count <= 2) && (err_count <= radio->low->fm_config.rds_error_limit))
	{
	...
	if ((block_type != RDS_BLKTYPE_E) || (radio->low->fm_state.save_eblks))
	{
		if (radio->rds_parser_enable)
		{
...
void fm_process_rds_data(struct s610_radio *radio)
{
	...
	for (i = 0; i < avail_blocks; i++)
	{
		...
		block_type =
			(fm_rds_block_type_enum)((status & RDS_BLK_TYPE_MASK) >> RDS_BLK_TYPE_SHIFT);
		...
		case RDS_STATE_HAVE_DATA:
			...
			if ((block_type == RDS_BLKTYPE_A) && (err_count == 0))
			{
#ifdef USE_RDS_BLOCK_SEQ_CORRECT
		...
		case RDS_STATE_FULL_SYNC:
			...
			if (fm_rds_process_block(radio,
									 data, block_type, err_count) == HOST_RDS_ERRS_UNCORR)
			{
...
void find_group_data(struct fm_rds_parser_info *pi, u16 info)
{
	...
	case RDS_GRPTYPE_2B:
		...
		if ((!segment && pi->rt_segment != 0xFF) || (pi->rt_change != 0xFF && pi->rt_change != rt_change))
		{
			validate_rt_data(pi);
			...
			for (i = 0; i < RDS_NUM_BLOCK_TYPES - 1; i++)
			{
				...
				if (fm_get_rds_sync_status(radio))
				{
					...
					if (fm_rds_process_block(radio, decoded[min_pos][min_blk][i].info,
											 decoded[min_pos][min_blk][i].blk_type, decoded[min_pos][min_blk][i].err_cnt) == HOST_RDS_ERRS_UNCORR)
					{
						fm_rds_update_error_status(radio,
												   radio->low->fm_state.rds_unsync_uncorr_weight);
					}
